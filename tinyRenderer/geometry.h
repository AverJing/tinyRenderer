...
template <typename T> struct vec<3,T> {
	...
	float norm() { return std::sqrt(x*x+y*y+z*z); } //可以使用快速开平方
...
template <typename T> struct vec<4, T> {
	...
	//vec<4, T> operator^(const vec<4, T> &v) const { return vec<4, T>(y*v.z - z * v.y, z*v.x - x * v.z, x*v.y - y * v.x); }
	T& operator[](const size_t i) { assert(i < 4); return i <= 0 ? x : (1 == i ? y : (2 == i ? z : w)); }
	const T& operator[](const size_t i) const { assert(i < 4); return i <= 0 ? x : (1 == i ? y : (2 == i ? z : w)); }
	float norm() { return std::sqrt(x*x + y * y + z * z + w * w); } //可以使用快速开平方
...
public:
    ...
    static mat<DimRows,DimCols,T> identity() {
        ...
        for (size_t i=DimRows; i--; )
            for (size_t j=DimCols;j--; ret[i][j]=(i==j));//很精简
        return ret;
    }
